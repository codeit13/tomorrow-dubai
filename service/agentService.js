...
agentRouter.put("/:id", async (req, res, next) => {
  ...
  try {
    ...
    const updatedAgent = await agentDB.findByIdAndUpdate(agentId, updates, {
      new: true,
    });
...
agentRouter.get("/:id", async (req, res, next) => {
  ...
  try {
    ...
    console.error("Error fetching agent:", error);
    res.status(500).json({
      error: "An error occurred while fetching agent id " + req.params.id,
    });
  }
});
...
agentRouter.delete("/:id", async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while deleting the agent with id" });
  }
});
